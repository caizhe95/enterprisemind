...
class ShortTermMemory:
    ...
    def __init__(self, max_tokens: int = 2000):
        ...
        self.compress_threshold = 6
        self.turn_count = 0.0
        self.summary = ""
        self.messages: List[BaseMessage] = []
        self.recent_raw: List[BaseMessage] = []

    def add_turn(self, human_msg: str, ai_msg: str):
        self.add_message(HumanMessage(content=human_msg))
        self.add_message(AIMessage(content=ai_msg))
    ...
    def _compress(self):
        """将较早的会话压缩成摘要，只保留最近两轮原文。"""
        if len(self.recent_raw) < 4:
            return

        to_compress = self.recent_raw[:-2]
        conv_text = "\n".join(
            [
                f"User: {m.content}"
...
新的累积摘要："""
        ...
        try:
            ...
            logger.info(f"[ShortTermMemory] 压缩完成: {self.summary[:50]}...")
        except Exception as e:
            logger.error(f"[ShortTermMemory] 压缩失败: {e}")
...
class LongTermMemory:
    ...
    def __init__(self, db_path: str = "./memory/long_term.db"):
        ...
        try:
            ...
            self.db_path = Path("./memory") / "long_term_fallback.db"
            self.db_path.parent.mkdir(parents=True, exist_ok=True)
            self._init_db()
...
结果："""
    ...
    def retrieve_relevant(self, user_id: str, query: str, top_k: int = 3) -> List[str]:
        ...
        keywords = set(query.lower().split()) - {"的", "了", "是", "什么", "怎么"}
...
class MemoryManager:
    ...
    def working(self) -> ShortTermMemory:
        """兼容旧调用：working -> short_term。"""
        return self.short_term

    @property
    def perceptual(self) -> ShortTermMemory:
        """兼容旧调用：perceptual -> short_term。"""
        return self.short_term
    ...
    def _load_user_context(self):
        with sqlite3.connect(self.long_term.db_path) as conn:
            ...
            row = cursor.fetchone()
            if row:
                self.short_term.summary = f"[上次对话: {row[0]}]"
    ...
    def update_turn(self, user_msg: str, ai_msg: str):
        ...
        if len(self.short_term.messages) % 4 == 0 and self.user_id:
            facts = self.long_term.extract_facts(self.user_id, self.short_term.messages)
            if facts:
    ...
    def finalize_session(self):
        ...
        summary = self.short_term.summary or "无摘要"
