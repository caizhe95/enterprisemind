...
- 多模态检索：接入 PDF/图片/表格解析，支持图文混合问答。
- Agent 能力扩展：新增 `planner/reviewer` 角色，提升复杂任务拆解与答案校验能力。
- 评测体系完善：引入真实业务问答集，补充端到端回归评测与自动告警。
- 知识库治理：增加文档版本管理、增量更新与失效数据清理策略。
- 观测与运维：接入链路追踪与指标面板，覆盖延迟、召回、成本、错误率。
- 安全与权限：细化 SQL 与工具调用权限，增加审计日志与敏感信息脱敏。
- 部署架构升级：支持多实例横向扩容、异步任务队列和高可用数据库方案。
